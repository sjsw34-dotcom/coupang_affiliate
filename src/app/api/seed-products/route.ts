...
// 카테고리별 검색 키워드 (각 20개 × 5개 상품 = 100개/카테고리)
...
function toSlug(keyword: string): string {
  ...
  return map[keyword] || keyword.replace(/\s+/g, '-').toLowerCase();
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Optional: seed only specific category
    const { searchParams } = new URL(req.url);
    const targetCategory = searchParams.get('category'); // e.g., 'electronics'
    ...
    for (const [categorySlug, keywords] of Object.entries(CATEGORY_KEYWORDS)) {
      ...
      if (!category) {
        results.push({ category: categorySlug, keyword: '', products: 0, collection: false, error: 'Category not found' });
        continue;
      }
      ...
      for (const keyword of keywords) {
        try {
          ...
          if (coupangResult.products.length === 0) {
            results.push({ category: categorySlug, keyword, products: 0, collection: false, error: 'No products found' });
            await delay(300);
            continue;
          }
          ...
          // Create collection for this keyword group
          const collSlug = `best-${toSlug(keyword)}-2025`;
          ...
          if (!existingCol) {
            const { data: collection, error: colError } = await supabase
              ...
              .insert({
                ...
                meta_description: `2025년 ${keyword} 순위. 스펙 비교 분석으로 선정한 추천 리스트.`,
    ...
    return NextResponse.json({
      ...
      summary: {
        totalProducts,
        totalCollections,
        byCategory: Object.keys(CATEGORY_KEYWORDS).map((cat) => {
          const catResults = results.filter((r) => r.category === cat);
          return {
            category: cat,
            products: catResults.reduce((sum, r) => sum + r.products, 0),
            collections: catResults.filter((r) => r.collection).length,
